...
const linkAction = () => {
    const navMenu = document.getElementById('nav-menu');
    // When we click on each nav__link, we remove the show-menu cick
    navMenu.classList.remove('show-menu');
}
...
const sendEmail = (e) => {
    ...
    emailjs.sendForm('service_42bx78y', 'template_60zz92q', '#contact-form', 'S-eDlnNBYWgkPBC_q')
    .then(() => {
        // Show sent message
...
sr.reveal(`.home__perfil, .about__image, .contact__mail, .skills__content:nth-child(2)`, {origin: 'right'})
sr.reveal(`.home__name, .home__info, .about__container .section__title-1, .about__info,
        .contact__social, .contact__data, .skills__content:nth-child(1)`, {origin: 'left'})
